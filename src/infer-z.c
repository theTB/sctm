...
void infer_z(sctm_data* data, sctm_params* params, sctm_latent* latent,
	...
	double p, p11, p12, p21, p22, r;
	double* P = (double*) malloc(sizeof(double) * params->K);
	int cnt;
	double p_beta, p_art, p_cmnt, p_v;
		...
		for (i = 0; i < doc->S; i++) {
			...
			for (n = 0; n < sent->N; n++) {
				...
				for (k = 0; k < params->K; k++) {
					...
					if (params->trte == 1) p_beta = latent->beta[k][v];
					else {
						...
						if (p21 <= 0) {
							printf("p21 phi_eta:%lf counts:%d", counts->phiEta_v[k], counts->n_dijv[k]);
							debug("in z");
						}
						p_beta = p11 / p21;
					}
					...
					if (params->CMNTS) {
						for (c = 0; c < doc->C; c++) {
							...
							if (isnan(p_cmnt) || isinf(p_cmnt)) {
								printf("d:%d c:%d i:%d n:%d k:%d m:%d m_1:%d m_k:%d p_cmnt:%lf\n",
										d,c,i,n,k,counts->m[d][c][k],counts->m_1[d][c][k], counts->m_k[d][c], p_cmnt);
								fflush(stdout);
								debug("chk");
							}
						}
					...
					if (isnan(p)) {
						printf("p:%lf,  p11:%lf, p21:%lf, p12:%lf, p22:%lf, p_cmnt:%lf p_v:%lf\n",
								p, p11, p21, p12, p22, p_cmnt, p_v);
						debug("in z");
					}
